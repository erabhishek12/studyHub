...
(function () {
  ...
  const CONFIG = {
    ...
    REQUEST_FORM_URL:
      "https://docs.google.com/forms/d/e/1FAIpQLSeE2QjRDeyfPm3CXezjlMPuzfE-nPhquPiPU_mPp2lfK8Vuww/viewform",

    // UPI Details
    UPI_ID: "9546983729@fam",
    ...
    LINK_PATTERNS: {
      ...
      drive: [/drive\.google\.com/i, /docs\.google\.com/i],
      github: [/github\.com/i, /raw\.githubusercontent\.com/i],
      pdf: [/\.pdf$/i, /\.pdf\?/i],
    },
  };
  ...
  async function startLoadingSequence() {
    ...
    try {
      ...
      if (cached) {
        ...
        hideLoadingScreen();
        showToast(
          "Using cached data - Some content may be outdated",
          "warning",
        );
      } else {
        updateLoadingUI({
  ...
  async function fetchAllData() {
    ...
    const fetchPromises = Object.entries(CONFIG.SHEETS).map(
      ([key, sheetName]) =>
  ...
  function toggleTheme() {
    ...
    applyTheme();
    showToast(
      user.darkMode ? "🌙 Dark mode enabled" : "☀️ Light mode enabled",
      "info",
    );
  }
  ...
  function loadSettingsData(user = null) {
    ...
    if (DOM.settingDarkMode)
      DOM.settingDarkMode.checked = user.darkMode || false;
  }
  ...
  function setupAnnouncement() {
    ...
    const announcement =
      state.data.config?.Announcement || state.data.config?.announcement;
  ...
  function toggleSidebar() {
    ...
    document.body.style.overflow = DOM.sidebar?.classList.contains("active")
      ? "hidden"
      : "";
  }
  ...
  function renderSearchResults(results) {
    ...
    DOM.searchResults
      ...
      .forEach((item) => {
        item.addEventListener("click", () =>
          handleSearchResultClick(item.dataset),
        );
      });
  }
  ...
  function handleSearchResultClick(data) {
    ...
    if (type === "tutorials") {
      ...
      const tutorial = (state.data.tutorials || []).find(
        (t) => (t.Topic || t.topic) === title || (t.ID || t.id) === id,
      );
      if (tutorial) {
  ...
  function updateResourceCounts() {
    if (DOM.notesCount)
      DOM.notesCount.textContent = state.data.notes?.length || 0;
    if (DOM.videosCount)
      DOM.videosCount.textContent = state.data.videos?.length || 0;
    if (DOM.pyqCount) DOM.pyqCount.textContent = state.data.pyq?.length || 0;
    if (DOM.tutorialsCount)
      DOM.tutorialsCount.textContent = state.data.tutorials?.length || 0;
  }

  // ==================== STREAK ====================
  function updateStreak() {
    const streak = getFromStorage(STORAGE_KEYS.STREAK) || {
      count: 0,
      lastVisit: null,
    };
  ...
  function showModal(title, message, onConfirm, icon = "⚠️", isDanger = false) {
    ...
    if (DOM.modalConfirm) {
      DOM.modalConfirm.className = isDanger
        ? "modal-btn danger"
        : "modal-btn primary";
  ...
  function renderCourseGrids() {
    ["notes", "videos", "pyq"].forEach((type) => {
      ...
      if (coursesGrid) {
        coursesGrid.innerHTML = CONFIG.COURSES.map(
          (course) => `
  ...
  function renderSemesters(type) {
    ...
    semestersGrid.innerHTML = semesters
      .map(
        (sem) => `
  ...
  function loadAndRenderResources(type) {
    ...
    setTimeout(() => {
      ...
      let resources = (state.data[type] || []).filter((item) => {
        ...
        const matchCourse =
          itemCourse === nav.course.toLowerCase() ||
          itemCourse.includes(nav.course.toLowerCase());
        const matchBranch =
          itemBranch === nav.branch.toLowerCase() ||
      ...
      const subjects = [
        ...new Set(
          resources.map((r) => r.Subject || r.subject).filter(Boolean),
        ),
      ];
  ...
  function filterAndRenderResources(type) {
    ...
    let resources = (state.data[type] || []).filter((item) => {
      ...
      return (
        matchCourse && matchBranch && matchSemester && matchSubject && notHidden
      );
    });
  ...
  function renderResourcePage(type) {
    ...
    resourceList.innerHTML = pageResources
      .map((resource) => createResourceCard(resource, type))
      .join("");
  ...
  function renderPagination(type, currentPage, totalPages, totalItems) {
    ...
    if (!paginationEl) {
      ...
      resourceList?.parentNode?.insertBefore(
        paginationEl,
        resourceList.nextSibling,
      );
    }
  ...
  function bindResourceCardEvents(container, type) {
    ...
    container.querySelectorAll(".resource-action-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        const title =
          card?.querySelector(".resource-title")?.textContent || "Resource";
  ...
  function updateBreadcrumb(type) {
    ...
    if (nav.semester)
      items.push({ label: `Semester ${nav.semester}`, icon: "📅" });
  ...
  function renderLanguageGrid() {
    ...
    languagesGrid.innerHTML = CONFIG.LANGUAGES.map(
      (lang) => `
  ...
  function getTutorialCount(language) {
    return (state.data.tutorials || []).filter(
      (t) =>
  ...
  function openTutorialReader(tutorial) {
    ...
    if (readerContent) {
      const content =
        tutorial.Content || tutorial.content || "No content available.";
  ...
  function formatTutorialContent(content) {
    if (!content)
      return '<div class="tutorial-empty"><p>No content available.</p></div>';
    ...
    html = html.replace(
      /^#### (.+)$/gm,
      '<h4 class="tutorial-h4"><span class="header-icon">📌</span> $1</h4>',
    );
    html = html.replace(
      /^### (.+)$/gm,
      '<h3 class="tutorial-h3"><span class="header-icon">📍</span> $1</h3>',
    );
    html = html.replace(
      /^## (.+)$/gm,
      '<h2 class="tutorial-h2"><span class="header-icon">📖</span> $1</h2>',
    );
    html = html.replace(
      /^# (.+)$/gm,
      '<h1 class="tutorial-h1"><span class="header-icon">📚</span> $1</h1>',
    );
    ...
    html = html.replace(
      /^&gt; (.+)$/gm,
      '<blockquote class="tutorial-quote"><span class="quote-icon">💡</span> $1</blockquote>',
    );
    ...
    html = html.replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" class="tutorial-link">$1 <span>↗</span></a>',
    );

    // Step 11: Images (if any)
    html = html.replace(
      /!\[([^\]]*)\]\(([^)]+)\)/g,
      '<figure class="tutorial-figure"><img src="$2" alt="$1" loading="lazy"><figcaption>$1</figcaption></figure>',
    );

    // Step 12: Note/Warning/Info boxes
    html = html.replace(
      /\[NOTE\]([\s\S]*?)\[\/NOTE\]/gi,
      '<div class="tutorial-note note"><span class="note-icon">📝</span><div class="note-content">$1</div></div>',
    );
    html = html.replace(
      /\[WARNING\]([\s\S]*?)\[\/WARNING\]/gi,
      '<div class="tutorial-note warning"><span class="note-icon">⚠️</span><div class="note-content">$1</div></div>',
    );
    html = html.replace(
      /\[TIP\]([\s\S]*?)\[\/TIP\]/gi,
      '<div class="tutorial-note tip"><span class="note-icon">💡</span><div class="note-content">$1</div></div>',
    );
    html = html.replace(
      /\[INFO\]([\s\S]*?)\[\/INFO\]/gi,
      '<div class="tutorial-note info"><span class="note-icon">ℹ️</span><div class="note-content">$1</div></div>',
    );
  ...
  function updateTutorialsBreadcrumb() {
    ...
    if (nav.topic)
      items.push({
        label: nav.topic.Topic || nav.topic.topic || "Tutorial",
        icon: "📖",
      });
  ...
  function shareCurrentTutorial() {
    ...
    const id =
      ...
      state.currentTutorial.topic;
    const title =
      state.currentTutorial.Topic || state.currentTutorial.topic || "Tutorial";
  ...
  function renderSavedItems(filter = "all") {
    ...
    let items = saved
      .map((item) => {
        ...
        if (item.type === "tutorials") {
          resource = (state.data.tutorials || []).find(
            (t) =>
          ...
          resource = (state.data[item.type] || []).find(
            (r) => String(r.ID || r.id) === String(item.id),
          );
          if (resource) {
    ...
    savedList.innerHTML = items
      .map((item) => createResourceCard(item, item._savedType))
      .join("");
  ...
  function handleRecentItemClick(id, type) {
    if (type === "tutorials") {
      navigateToPage("tutorials");
      const tutorial = (state.data.tutorials || []).find(
        (t) => String(t.ID || t.id || t.Topic || t.topic) === String(id),
      );
      if (tutorial) {
        selectLanguage(tutorial.Subject || tutorial.subject);
        setTimeout(() => openTutorialReader(tutorial), 300);
      ...
      const resource = (state.data[type] || []).find(
        (r) => String(r.ID || r.id) === String(id),
      );
      if (resource) {
  ...
  function processShareLink(shareData) {
    const { type, id, highlight, course, branch, sem, language, topic } =
      shareData;
  ...
  function openSharedTutorial(id, highlight, language) {
    ...
    setTimeout(() => {
      ...
      if (tutorial) {
        ...
        // Select language
        const tutorialLang =
          language || tutorial.Subject || tutorial.subject || tutorial.Language;
  ...
  function initTodoTool() {
    ...
    filterBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        renderTodos(
          getFromStorage(STORAGE_KEYS.TODOS) || [],
          btn.dataset.filter,
        );
      });
    });
    ...
    if (filteredTodos.length === 0) {
      ...
      list.querySelectorAll(".todo-item").forEach((item) => {
        ...
        item
          ...
          ?.addEventListener("click", () => toggleTodoItem(id));
        item
          .querySelector(".todo-delete")
          ?.addEventListener("click", () => deleteTodoItem(id));
      });
    }
  ...
  function initPomodoroTool() {
    ...
    // Set input values
    document.getElementById("pomodoro-focus-input").value =
      state.pomodoro.focusTime;
    document.getElementById("pomodoro-short-input").value =
      state.pomodoro.breakTime;
    document.getElementById("pomodoro-long-input").value =
      state.pomodoro.longBreakTime;
    ...
    [
      ...
      "pomodoro-short-input",
      "pomodoro-long-input",
    ].forEach((id) => {
      document
        .getElementById(id)
        ?.addEventListener("change", updatePomodoroSettingsFromInputs);
    });
  ...
  function updatePomodoroSettingsFromInputs() {
    state.pomodoro.focusTime =
      parseInt(document.getElementById("pomodoro-focus-input")?.value) || 25;
    state.pomodoro.breakTime =
      parseInt(document.getElementById("pomodoro-short-input")?.value) || 5;
    state.pomodoro.longBreakTime =
      parseInt(document.getElementById("pomodoro-long-input")?.value) || 15;

    if (!state.pomodoro.isRunning) {
      const activeMode =
        document.querySelector(".pomodoro-mode.active")?.dataset.mode ||
        "focus";
  ...
  function handlePomodoroComplete() {
    ...
    if (state.pomodoro.isFocus) {
      ...
      state.pomodoro.timeLeft = shouldLongBreak
        ? state.pomodoro.longBreakTime * 60
        : state.pomodoro.breakTime * 60;
      document.getElementById("pomodoro-status").textContent = shouldLongBreak
        ? "🌴 Long Break"
        : "☕ Short Break";
      ...
      document.querySelectorAll(".pomodoro-mode").forEach((btn) => {
        btn.classList.toggle(
          "active",
          btn.dataset.mode === (shouldLongBreak ? "long" : "short"),
        );
      });
    } else {
  ...
  function updatePomodoroDisplay() {
    ...
    if (progressEl) {
      const totalTime = state.pomodoro.isFocus
        ? state.pomodoro.focusTime * 60
        : (state.pomodoro.sessions % 4 === 0
            ? state.pomodoro.longBreakTime
            : state.pomodoro.breakTime) * 60;
  ...
  function calculateCgpa() {
    ...
    rows.forEach((row) => {
      ...
      const credits =
        parseFloat(row.querySelector(".cgpa-credits")?.value) || 0;
    ...
    if (totalCredits === 0) {
      showToast(
        "Please enter SGPA and credits for at least one semester",
        "error",
      );
      return;
    }
  ...
  function calculatePercentage() {
    ...
    const scale =
      document.querySelector('input[name="gpa-scale"]:checked')?.value || "10";
    const formula =
      document.getElementById("percentage-formula")?.value || "standard";
  ...
  function calculateAttendance() {
    const total =
      parseInt(document.getElementById("total-classes")?.value) || 0;
    const attended =
      parseInt(document.getElementById("attended-classes")?.value) || 0;
  ...
  function updateCalcDisplay(currentEl, historyEl) {
    ...
    if (historyEl) {
      historyEl.textContent = calc.operator
        ? `${calc.previous} ${opSymbols[calc.operator] || calc.operator}`
        : "";
    }
  }
  ...
  function calculateAge() {
    ...
    document.getElementById("age-total-days").textContent =
  ...
  function renderQuickNotes(notes) {
    ...
    if (notes.length === 0) {
      ...
      list.querySelectorAll(".quicknote-card").forEach((card) => {
        const id = card.dataset.id;
        card
          .querySelector(".quicknote-delete")
          ?.addEventListener("click", () => deleteQuickNote(id));
  ...
  function initUnitConverterTool() {
    ...
    document
      .getElementById("unit-from-unit")
      ?.addEventListener("change", convertUnits);
    document
      .getElementById("unit-to-unit")
      ?.addEventListener("change", convertUnits);
  }
  ...
  function saveSettings() {
    ...
    user.college =
      document.getElementById("setting-college")?.value.trim() || "";
    ...
    user.examName =
      document.getElementById("setting-exam-name")?.value.trim() || "";
